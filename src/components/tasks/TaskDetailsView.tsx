...
export default function TaskDetailsView({ 
  ...
  const isDelegator = user && task.delegator_id === user.id;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-auto">
        <div className="p-6">
          ...
          {/* Action message with appropriate styling */}
          {actionMessage && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
              ...
              <p>{task.delegator?.full_name || 'Unknown'} ({task.delegator?.email || 'No email'})</p>
            </div>
            
            <div>
              <h3 className="text-sm font-medium text-gray-500 mb-1">Assigned To</h3>
              <p>{task.assignee ? `${task.assignee.full_name || 'Unknown'} (${task.assignee.email || 'No email'})` : 'Unassigned'}</p>
            </div>
